...
public class NetworkStarter : MonoBehaviour
{
    ...
    private void StartNetworkStandard() {
        ...
        if (_networkManager.transport is SteamTransport && !_lobbyDataHolder.CurrentLobby.IsOwner) {
            string lobbyIdStr = _lobbyDataHolder.CurrentLobby.LobbyId;
            CSteamID steamLobbyId = new CSteamID(Convert.ToUInt64(lobbyIdStr));

            CSteamID hostId = SteamMatchmaking.GetLobbyOwner(steamLobbyId);

            Debug.Log("Lobby host SteamID64: " + hostId.m_SteamID);

            // Now set this as the address for SteamTransport
            (_networkManager.transport as SteamTransport).address = hostId.m_SteamID.ToString();
        }
