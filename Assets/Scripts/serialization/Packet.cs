...
public class Packet {
    ...
    public void WriteList<T>(List<T> list) where T : IByteSerialization { 
		Write(list.Count);
	...
	public void Write(IByteSerialization pSerializable) {
	...
	public List<T> ReadList<T>() {
		...
		int count = ReadInt();
		while (count > 0) {
			toReturn.Add((T)ReadObject());
			count--;
		}
		return toReturn;
	}
	public Vector3 ReadVec3() { return new Vector3(ReadFloat(), ReadFloat(), ReadFloat()); }
    public byte[] readByteArray() {
		int len = ReadInt();
		byte[] toReturn = new byte[len];
		int i = 0;
		while (i < len) {
            toReturn[i] = reader.ReadByte();
			i++;
		}
		return toReturn;
    }

    public IByteSerialization ReadObject() {
		Type type = Type.GetType(ReadString());
		IByteSerialization obj = (IByteSerialization)Activator.CreateInstance(type);
		obj.Deserialize(this);
		return obj;
	}

	public T Read<T>() where T : IByteSerialization {
		return (T)ReadObject();
	}
